#include "LipmDescription.h"


void LipmDescription::getTransDistrib(Distributions::Distrib& P, Value& Xt, Value& U)
{
...
//vector<double> LipmDescription::learnParams(Value& Xt, Value& U, unsigned int nb_sample_per_dim)
//{
//    cXt.continuize(Xt);
//    cU.continuize(U);
...
LipmDescription::~LipmDescription()
{
        //dtor
}

void LipmOneGaussDescription::getTransDistrib(Distributions::Distrib& P, Value& Xt, Value& U)
{
    cXt.continuize(Xt);
    cU.continuize(U);
    sim.setXU(cXt,cU);
...
void LipmSimulator::simulate(ContValue& X, ContValue& U, double dt)
{
    ...
    //FIXME
    if(U[1]==0.0) //pas de changement d'appui
    {
...
void LipmManager::buildVariables()
{
...
void LipmManager::buildSimulator()
{
    sim = new LipmSimulator(*state, *action, DT, XMIN, XMAX);
}

void LipmManager::buildModel()
{
    model = new LipmDescription(*state, *action, *sim);
}

void LipmManager::buildOptimizer(bool drawnPolicy, double initBeta)
{
    optimizer = new OptimizerQuasiDist(*model, drawnPolicy, initBeta);
}

void LipmManager::buildDistribLearner()
{
...
//            plotPosDiscr.clear(); plotSpeedDiscr.clear(); plotActionDiscr.clear();
       ...
       //         cout<<t<<" ; ("<<Xt[0]<<", "<<Xt[1]<<", "<<U[0]<<")  ;  ("<<Xt_c[0]<<", "<<Xt_c[1]<<", "<<U_c[0]<<")"<<endl;
               ...
               // dynamic_cast<LipmSimulator*>(sim)->setFriction(0.2);
          //      dynamic_cast<LipmSimulator*>(sim)->setSlope(0.18);
