...
void NonHoloSimulator::simulate(ContValue& X, ContValue& U, double dt)
{
    ...
    for(int i=0;i<100;i++)
    {
        xt+=cos(thetat)*dt/100.0*U[0];
        yt+=sin(thetat)*dt/100.0*U[0];
        thetat+=U[1]*dt/100.0;
    }
...
void NonHoloManager::buildVariables()
{
    ...
    actionCards[0]=U0CARD; actionMins[0]=U0MIN; actionMaxs[0]=U0MAX; actionCircular[0]=false;
    actionCards[1]=U1CARD; actionMins[0]=U1MIN; actionMaxs[0]=U1MAX; actionCircular[0]=false;
