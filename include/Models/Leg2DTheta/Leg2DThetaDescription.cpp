#include "Leg2DThetaDescription.h"


void Leg2DThetaDescription::getTransDistrib(Distributions::Distrib& P, Value& Xt, Value& U)
{
...
Leg2DThetaDescription::~Leg2DThetaDescription()
{
        //dtor
}

void Leg2DThetaOneGaussDescription::getTransDistrib(Distributions::Distrib& P, Value& Xt, Value& U)
{
    Distributions::Distrib1D p_theta, p_dtheta,p_h, p_dh;
    cXt.continuize(Xt);
    cU.continuize(U);
...
void Leg2DThetaSimulator::simulate(ContValue& X, ContValue& U, double dt)
{
    ...
    double J12=-(-L1 + L2)*(-0.5*cotan(0.5*beta)*cotan(0.5*beta) - 0.5)/((L1 + L2)*((-L1 + L2)*(-L1 + L2)*cotan(0.5*beta)*cotan(0.5*beta)/(L1 + L2)*(L1 + L2) + 1)) + 0.5;
        ...
        //double J21=0;
    double J22=L1*L2*sin(beta)/sqrt(L1*L1 - 2*L1*L2*cos(beta) + L2*L2);
    ...
    double dthetat=dalpha+dbeta*J12;
...
void Leg2DThetaManager::buildVariables()
{
...
void Leg2DThetaManager::buildSimulator()
{
        //sim = new Leg2DThetaSimulator(*state, *action, DT, XMIN, XMAX);
    sim = new Leg2DThetaSimulator(*state, *action, DT);
}

void Leg2DThetaManager::buildModel()
{
    model = new Leg2DThetaDescription(*state, *action, *sim);
}

void Leg2DThetaManager::buildOptimizer(bool drawnPolicy, double initBeta)
{
    optimizer = new OptimizerQuasiDist(*model, drawnPolicy, initBeta);
}

void Leg2DThetaManager::buildDistribLearner()
{
...
//            plotPosDiscr.clear(); plotSpeedDiscr.clear(); plotActionDiscr.clear();
       ...
       //         cout<<t<<" ; ("<<Xt[0]<<", "<<Xt[1]<<", "<<U[0]<<")  ;  ("<<Xt_c[0]<<", "<<Xt_c[1]<<", "<<U_c[0]<<")"<<endl;
                ...
                plotDAlpha.push_back(U_c[0]);
                    //plotDBeta.push_back(U_c[1]);
                
                    

               // dynamic_cast<Leg2DThetaSimulator*>(sim)->setFriction(0.2);
          //      dynamic_cast<Leg2DThetaSimulator*>(sim)->setSlope(0.18);
...
//
        ...
        {
            file<<plotTheta[t]<<" "<<plotDTheta[t]<<" "<<plotH[t]<<" "<<plotDH[t]<<" "<<plotDAlpha[t]/*<<" "<<plotDBeta[t]*/<<endl;
        }
        file.close();
        ...
        for (unsigned int t=0; t<max_t_plot; t++)
        {
            file_discr<<plotThetaD[t]<<" "<<plotDThetaD[t]<<" "<<plotHD[t]<<" "<<plotDHD[t]<<" "<<plotDAlphaD[t]/*<<" "<<plotDBetaD[t]*/<<endl;
        }
...
Leg2DThetaManager::~Leg2DThetaManager()
{
    //dtor
}
